        ...
        const int line)
{
  ...
  if ( name != list->name )
  {
    ...
    snprintf(tmp42, 0x1000u, v13, v12, "_k_insert_before", name, file, func, line, v11, "_k_insert_before", 262);
    applog(3, tmp42, 1);
    quit(1);
  }
  if ( !before )
  {
    ...
    LOWORD(v17) = 27756;
    HIWORD(v17) = (unsigned int)" do_tail is false - from %s %s() line %d in %s %s():%d" >> 16;
    snprintf(tmp42, 0x1000u, v17, "_k_insert_before", list->name, file, func, line, v16, "_k_insert_before", 267);
    applog(3, tmp42, 1);
    quit(1);
  }
  item->next = before;
