void __fastcall stage_work(work *work)
{
  ...
  if ( pthread_mutex_lock(v2) )
  {
    ...
    HIWORD(v68) = (unsigned int)"RROR ON LOCK! errno=%d in %s %s():%d" >> 16;
    snprintf(tmp42, 0x1000u, v68, *v66, v67, "hash_push", 5533);
LABEL_102:
    applog(3, tmp42, 1);
    _quit(1, 1);
  }
...
LABEL_90:
  ...
  if ( pthread_mutex_unlock(stgd_lock) )
  {
    ...
    LOWORD(v64) = -920;
    LOWORD(v65) = -3136;
    HIWORD(v64) = (unsigned int)"X ERROR ON LOCK! errno=%d in %s %s():%d" >> 16;
    HIWORD(v65) = (unsigned int)"iner" >> 16;
    snprintf(tmp42, 0x1000u, v64, *v63, v65, "hash_push", 5544);
    goto LABEL_102;
  }
  selective_yield();
}
