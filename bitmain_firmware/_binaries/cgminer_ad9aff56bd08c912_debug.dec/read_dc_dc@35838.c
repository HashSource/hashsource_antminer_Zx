...
int __fastcall read_dc_dc(uint8_t i2c_dev_addr, uint8_t reg, uint8_t *value, int len)
{
  ...
  int v16; // r3
  int v17; // r2
  int v18; // [sp+14h] [bp-101Ch] BYREF
  char v19; // [sp+18h] [bp-1018h]
  uint8_t rega; // [sp+19h] [bp-1017h]
  char v21; // [sp+1Ah] [bp-1016h]
  char v22; // [sp+1Bh] [bp-1015h]
  char v23; // [sp+1Ch] [bp-1014h] BYREF
...
LABEL_9:
    if ( !use_syslog && !opt_log_output && opt_log_level <= 4 )
      return -1;
    LOWORD(v15) = -30884;
    HIWORD(v15) = (unsigned int)"%s failed! read_back_data[0] = 0x%02x, read_back_data[1] = 0x%02x, read_back_data[2] = 0x%02x, read_back_data[3] = 0x%02x\n\n" >> 16;
    snprintf(tmp42, 0x1000u, v15, "read_dc_dc", read_back_data[0], v13, read_back_data[2], read_back_data[3]);
    applog(5, tmp42, 0);
    return -1;
  }
  do
  ...
  if ( len > 0 )
  {
    ...
    do
    {
      value[v16] = read_back_data[v17 + 3];
