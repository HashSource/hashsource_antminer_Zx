void __fastcall hash_sole_work(thr_info *mythr)
{
  ...
  int v35; // [sp+3Ch] [bp-1038h]
  int v36; // [sp+40h] [bp-1034h]
  ...
  do
  {
    ...
    *p_device_diff = max_diff;
        ...
        && work_difficulty > working_diff )
      {
        ...
        if ( HIDWORD(v12) )
        {
          if ( use_syslog || opt_log_output || (HIDWORD(v12) = opt_log_level, opt_log_level > 6) )
          {
            ...
            v26 = drv->working_diff;
          }
        }
        if ( *p_work_difficulty < v26 )
          v26 = *p_work_difficulty;
        *p_device_diff = v26;
      }
    ...
    while ( 1 )
    {
      ...
      {
        max_nonce = (max_nonce << 10) / ((v35 + shutdown) / v36);
      }
